...
function upload(file) {
	...
	if(xhr.readyState==4)
		{
            ...
            if (status == "0") {
                status = "Uploaded OK.  Please Wait";
            }
			$('#status').html(status).css('background-color', 'orange');
	...
	xhr.ontimeout = function () { 
		alert("Sorry.  Your upload timed out.  Try again");
		$('#submit_button').unbind('click');
		$('#submit_button').css('background-color', submitDefaultColor).text('Upload Booth').css('cursor','pointer');
		$('#submit_button').bind('click',upload);
		$('#progress').attr('value', 0);		
	};
	xhr.open('POST', '/newbooth/file_upload', true);
	xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	var data = 'image=' + dataURL + '&blurb=' + encodeURIComponent($('#blurb').val());
	try {
		xhr.send(data);
	} catch (e) {
		$('#submit_button').unbind('click');
