...
function openBooth(boothnum) {
    ...
    $("#centerpane").html(
        "<div class = \"centersection\" id=\"boothtop\"></div>" +
            ...
            "<iframe class = \"centersection\" id=\"comment_target\" name = \"comment_target\" src = \"/comment/target.html\"  style=\"width:100%;height:0;border:0 solid #FFF;\"></iframe>" +
...
//    console.debug("Opening comment input section for booth " + boothnum);
    $.post("/booth/getboothcommentinputsection.php", {
        boothnum: boothnum
    }, function (data) {
...
function getBoothHTML(data) {
    ...
    $.each(data, function (idx, obj) {
        ...
        var newLocation = window.location.pathname.split('/')[0] + "/users/" + obj.boothername + "/" + obj.boothnum;
//        console.debug("Curloc: " + window.location.pathname + " newLoc: " + newLocation);
        ...
        if (window.username == obj.boothername) {
            html = html +
                ...
                "<a href = '/actions/deletebooth?number=" + obj.boothnum + "'><div class = 'boothbutton'>Delete</div></a>";
        }
        html = html +
            "<div style = \"clear: both\"></div>" +
            "</div>" +
...
function getBoothCommentsHTML(boothnum, data) {
    ...
    $.each(data, function (idx, obj) {
        ...
        if (hasImage) {
            html = html +
                "<div class = \"boothcommenterimageuploaded\" style = \"background-image: url(" + obj.imageHash + "); height: " + imageHeightInEM + "em;\"></div>";
        } else {
            html = html +
                "<div class = 'boothcommenterimage' onclick=\"openUserFeed('" + obj.commentername + "')\" style = 'background-image: url(" + obj.iconImage + ")'></div>";
        }
        var style = "display: none;";
        if ("undefined" !== typeof(obj.likes) && obj.likes != null && obj.likes > 0) {
            style = "display: inherited;";
